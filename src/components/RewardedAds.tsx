...
const RewardedAds: React.FC<{
  ...
  const handleAd = async () => {
    ...
    try {
      ...
      if (res.data.allowed) {
        ...
        alert("🎯 You’ve reached your daily limit of 20 ads.");
      }
    } catch (error) {
      console.error(error);
    ...
    (async () => {
      if (userID) {
        try {
          ...
          setAdCounter(
            response.data.viewsLeft ? 20 - response.data.viewsLeft : 0
          );
        } catch {}
      }
    })();
  ...
  return (
    <div className="bg-[#ac3fa3] rounded-lg p-4 mt-4">
      ...
      <p className="text-sm">
        <span className="font-bold">{adCounter}/20</span> ads viewed today
      </p>
      <button
        onClick={handleAd}
